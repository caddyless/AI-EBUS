...
class VideoPlayer(object):
    ...
    def show_representative(self, mode, data_folder):
        aggregator = self.clf_model.branches[mode].aggregator.aggregator

        frame_path = Path(data_folder) / 'E' / 'elastic'

        raw_data = []
        file_path = []
        for file in frame_path.iterdir():
            img = Image.open(str(file))
            img = self.data_format(np.array(img), 'E')
            raw_data.append(img.unsqueeze(0))
            file_path.append(file)

        raw_data = torch.cat(raw_data, dim=1).cuda()

        feature = self.clf_model.branches[mode].extract(raw_data)
        aggregated, weights = aggregator(feature, torch.ones((1, 1), dtype=torch.int8, device='cuda') * raw_data.size(1),
                                            with_weights=True)
        representative_indices = torch.argsort(weights.squeeze(), descending=True)
        representative_indices = representative_indices.cpu().tolist()
        ranked = []
        for r in representative_indices:
            ranked.append(file_path[r])
...
if __name__ == '__main__':
    vp = VideoPlayer(video_dir='/data/tidy-data/20190325梅蓉4R',
                     hospital='shanghai',
                     datatype_arrange={'B': 'raw-img', 'F': 'ROI', 'E': 'elastic-hist'},
                     mode='BFE',
                     clf_model_folder='../task-checkpoint/video/classification-2020-12-28 00:54:20/4/model')
    
    # patients = [4, 10, 13, 24, 40, 42, 59, 90, 89, 96, 107, 133, 162, 166]
    patients = [115]

    root_dir = '/data/data/video/mm-ete/20210107-whole-validation'
    root_dir = Path(root_dir)

    target_dir = '/data/data/representative/'
    ...
    for index in bar:
        for folder in root_dir.iterdir():
            if folder.name.startswith('%d-' % index):
                print('Matched ', folder.name)
                target_folder = target_dir / folder.name
                target_folder.mkdir(exist_ok=True)
                ranked = vp.show_representative('E', str(folder))

                for i in range(3):
                    shutil.copy(ranked[i], str(target_folder / os.path.basename(ranked[i])))
