...
class NASNetAMobile(nn.Module):
    ...
    def __init__(self, in_channel=3, num_classes=1000, stem_filters=32, penultimate_filters=1056, filters_multiplier=2):
        ...
        self.conv0.add_module('conv', nn.Conv2d(in_channels=in_channel, out_channels=self.stem_filters, kernel_size=3, padding=0, stride=2,
                                                bias=False))
...
def nasnetamobile(in_channel=3, num_class=2, pretrained=None):
    r"""NASNetALarge model architecture from the
    `"NASNet" <https://arxiv.org/abs/1707.07012>`_ paper.
    """
    if pretrained:
        settings = pretrained_settings['nasnetamobile'][pretrained]
        assert num_class == settings['num_classes'], \
            "num_classes should be {}, but is {}".format(settings['num_classes'], num_class)

        # both 'imagenet'&'imagenet+background' are loaded from same parameters
        model = NASNetAMobile(in_channel=in_channel, num_classes=num_class)
