...
class Causal_Norm_Classifier(nn.Module):
    ...
    def __init__(self, num_classes=1000, feat_dim=2048, use_effect=True, num_head=2, tau=16.0, alpha=3.0, gamma=0.03125,
        ...
        self.mu = mu
        self.embed = torch.zeros((1, feat_dim), dtype=torch.float, device='cuda')
    ...
    def forward(self, x):
        ...
        if self.training:
            self.embed = self.mu * self.embed + (1 - self.mu) * x.mean(0, keepdim=True)
...
def create_model(feat_dim, num_classes=1000, stage1_weights=False, dataset=None, log_dir=None, test=False,
                 use_effect=True, num_head=2, tau=16.0, alpha=3.0, gamma=0.03125, *args):
