...
```{r}
cm = table(y_true, y_predict)
  result = epi.tests(cm, conf.level=0.95)
  re_key = c('diag.acc', 'se', 'sp', 'ppv', 'npv')
  for (j in 1:5){
    k = re_key[j]
    ele = result$rval[[k]]
    out_content = paste(out_content, ' ', k, ' ', as.character(ele[1]), '(', as.character(ele[2]), ',', as.character(ele[3]), ')', sep='')
  }
  print(out_content)
